...
def main():
    ...
    ap.add_argument("--brand", help="the brand folder under brands/ — read off the "
                                    "run when it recorded one, never defaulted")
    a = ap.parse_args()
    ...
    if a.packshot:
        ...
        # The packshot comes from the brand the run belongs to. This searched
        # one brand's products whatever the run was, so every other brand's ad
        # was built with the wrong product on it, or none.
        brand = P.brand_of_run(run, a.brand)
        cands = sorted(P.brand(brand)["products"].rglob("*.png"))
