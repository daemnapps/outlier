...
**THE PRODUCT ON THIS PAGE IS THE ONE IN THE PRODUCT FILE — and only that one.** Read the product file first and hold it above everything else you were given. The source page sold a different product; the brand context may describe sister products; the avatar's file may name the body parts a sister product is for. None of that transfers. Every product name, every ingredient, every exfoliant, every body part, every usage instruction, every claim and every FAQ on this page is the product file's, said the way the product file says it. Where the source's product and ours differ — where it is used, what it is made of, how it feels, who it is for — the slot is filled from the product file, and the source's version is dropped without comment. If the product file does not say something the slot needs, the slot is `[UNFILLED]`; it is never filled from a sister product's file. A review, a customer line or a proof point that is plainly about a sister product (it names a body part or an ingredient this product does not have) is not reused here — say so in the substitution log and leave the slot `[UNFILLED]`. Finish by checking: does any line on this page describe the source's product rather than ours? If yes, fix it before you answer.
