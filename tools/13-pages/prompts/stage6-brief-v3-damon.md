...
The seven named techniques, for technique-per-block below: {techniques}
...
- **Whose voice it is in** — the narrator persona, and that the narrator is a cast persona and not a customer. **The narrator is never the source page's byline.** The swipe's writer is theirs; ours is whoever triage and the page itself bound the voice to. If the page's own byline slot is unfilled, say `narrator: unnamed first-person — cast persona to be chosen` and carry the byline decision into BEFORE YOU BUILD IT.
...
page_next:    {page_next}
narrator:     <the cast persona's name, or "unnamed first-person — cast persona to be chosen"; never the source page's byline>
```
...
**TECHNIQUE PER BLOCK** — per block above, one line: the block · the ONE
technique from the seven above doing its work (intensification,
identification, gradualization, redefinition, mechanization, concentration,
camouflage) · the specific line carrying it. A block with no traceable
technique says so rather than naming one to fill the row. This is a reading
of the finished page, not an instruction to change it.
