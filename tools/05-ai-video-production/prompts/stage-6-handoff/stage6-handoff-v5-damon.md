...
*v5 (2026-09-22): FORMATS rewritten — one ratio. Nothing is made at 4:5
or 1:1; every frame keeps faces, product and words inside the centred 4:5
crop of the 9:16 frame, and the ask is the 15-second cutdown map plus that
check. Damon: "we shouldn't create content that's 1:1".*

*v4 (2026-09-22): one new section after THE LOOP — THE ASKS, the six
things the editor or designer makes beyond the base cut (scroll stoppers,
headlines, variations, extra scenes, formats, styles), each specified or
refused with a reason. The list and its defaults: `../../ASKS-SPEC.md`. No
new variables.*
...
What the editor or designer makes beyond the base cut. Exactly six lines,
in this order, each either specified — a count and what drives it — or
`none — <why>`. Never a blank line: a blank is a decision nobody made, and
the editor will fill it with a guess.

- **SCROLL STOPPERS** — from the production document's hook set: every
  hook that is not already the primary opener becomes one alternative
  first three seconds, same scene-1 setting. Name each hook.
- **HEADLINES** — on-screen lines for the opener card, under eight words,
  taken from the hooks, THE LOOP and the offer as written. List them.
- **VARIATIONS** — alternate takes, same line, different framing: the
  opening scene and the offer scene unless the document says otherwise.
- **EXTRA SCENES** — every beat that POST or KNOWN ISSUES leaves
  uncovered, as an insert to generate from the piece's setting and product.
  If nothing is uncovered, `none — every beat is carried`.
- **FORMATS** — 9:16 only; nothing is made at 4:5 or 1:1. Two things: the
  15-second cutdown map (which scenes survive, in order), and one line per
  scene where a face, the product or a word on screen sits outside the
  centred 4:5 window — the middle 70% of the frame — because that scene is
  cut off in feed. If every scene is clear, `all scenes inside the 4:5 window`.
- **STYLES** — only a style variation the production document names.
  Otherwise `none — no style was asked for`.

Every ask keeps the cast references, the product references, the wardrobe
locks, the light and the skin rule of the base set. An ask that needs a
new person, a new product fact or a new claim is not an ask; leave it out
and say so under KNOWN ISSUES.
