#!/usr/bin/env python3
"""Make the plates a brief asks for, on fal.
...
The original is kept verbatim at `image-production/tools/superseded/generate.py` so an old run's records still read.
Nothing imports it.
...
sys.exit(
    "generate.py is retired — it ran on fal, and fal is not a generation path any "
    "more (Damon, 2026-09-11, restated 2026-09-13).\n"
    "  a batch:     image-production/run.py runs/<brand>/<batch>\n"
    "  variations:  image-production/tools/make_variations.py\n"
    "  one edit:    image-production/tools/make_variations.py\n"
    "The original is parked at image-production/tools/superseded/generate.py.")
